package App::Yath::Schema::Dumper;
use strict;
use warnings;
...
sub dump {
    ...
    for my $source (@DUMP_ORDER, $schema->sources) {
        ...
        while ($page <= $pages) {
            my $file = "${dir}/${source}-" . sprintf("%0${len}d", $page) . ".jsonl.bz2";
            die "Dump file '$file' already exists!\n" if -e $file;

            $runner->run(sub {
                print "$$ $source STARTED ($page/$pages) -> $file\n";
                my $fh = IO::Compress::Bzip2->new("${dir}/${source}-" . sprintf("%0${len}d", $page) . ".jsonl.bz2") or die "Could not open log file: $IO::Compress::Bzip2::Bzip2Error";
