...
subtest 'TO_JSON round-trips through json_fields' => sub {
    my $tf = Test2::Harness2::TestFile->new(
        file      => '/abs/t/foo.t',
        min_slots => 3,
        category  => 'immiscible',
    ...
    ok(!exists $json->{absolute}, 'derived fields omitted');
    ok(!exists $json->{relative}, 'derived fields omitted');

    my $rebuilt = Test2::Harness2::TestFile->new(%$json);
...
subtest 'defaults() returns fresh mutable values per call' => sub {
    my $a = Test2::Harness2::TestFile->defaults;
    my $b = Test2::Harness2::TestFile->defaults;
    ...
    is($b->{conflicts}, [], 'conflicts arrayref is fresh per call');
    is($b->{features},  {}, 'features hashref is fresh per call');
    is($b->{switches},  [], 'switches arrayref is fresh per call');
    is($b->{meta},      {}, 'meta hashref is fresh per call');
};
