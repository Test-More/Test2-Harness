...
sub handle_file {
    ...
    unless (eval { require $rel; 1 }) {
        my $err = $@;
        if ($err =~ m/deprecated/i) {
            $pod = generate_deprecated_pod($pkg);
        }
        else {
            warn "Could not load $pkg, emitting stub POD:\n$err";
            $pod = generate_unavailable_pod($pkg);
        }
    }
...
sub generate_unavailable_pod {
    my $class = shift;

    my @out = (
        "=head1 NAME",
        "$class - POD unavailable",
        "=head1 DESCRIPTION",
        "This command's documentation could not be auto-generated because the "
            . "module failed to load at build time. See the build log for the "
            . "underlying error.",
    );
