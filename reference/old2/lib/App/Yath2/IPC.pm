...
use Test2::Harness2::Util qw/mod2file clean_path fqmod/;
use Test2::Harness2::IPC::Util qw/pid_is_running/;
...
sub _load_protocol {
    ...
    my $prot   = fqmod($in, 'Test2::Harness2::IPC::Protocol');
    my $pshort = $prot =~ m/^Test2::Harness2::IPC::Protocol::(.+)$/ ? $1 : "+$prot";
...
sub start {
    ...
    my $ipc = Test2::Harness2::IPC::Protocol->new(protocol => $specs->{protocol});
...
sub _generate_ipc {
    ...
    my $prot   = $settings->ipc->protocol // 'Test2::Harness2::IPC::Protocol::AtomicPipe';
    my $pshort;
