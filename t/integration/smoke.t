...
subtest single_proc => sub {
    my $out = yath(command => 'test', pre => ['-p+SmokePlugin'], args => [$dir, '--ext=tx'], log => 1, debug => 0);
...
subtest multi_proc => sub {
    my $out = yath(command => 'test', pre => ['-p+SmokePlugin'], args => [$dir, '-j3', '--ext=tx'], log => 1, debug => 0);
    ok(!$out->{exit}, "exit success");
    my $log = $out->{log};
