...
if ($ENV{TEST2_HARNESS_ACTIVE}) {
    # Nothing currently -- yath already allocates an isolated temp
    # dir per test so this plugin would be redundant inside a yath
    # run. Left as a no-op branch to match old/ and to keep the
    # plugin safe to `use` unconditionally.
}
else {
