...
use Test2::Harness2::Util qw/find_libraries clean_path mod2file read_file/;
...
sub cli_help {
    ...
    if ($cmd_class) {
        ...
        $no_cmd = 1;
        require App::Yath2::Command::help;
...
sub options {
    ...
    $self->{+OPTIONS}->include(App::Yath2::Options::Yath->options);
        ...
        (
            map {
                eval { require(mod2file($_)); 1 }
                    ? [$_ => $_->VERSION // 'N/A']
                    : [$_ => 'N/A']
            } qw/Test2::API Test2::Suite Test::Builder Test2::Harness2/,
        ),
        (
            map {
