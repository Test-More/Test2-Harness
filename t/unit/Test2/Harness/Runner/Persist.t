...
sub spawn_stage {
    ...
    my $guard = Scope::Guard->new(sub {
        confess "spawn_stage($stage) was about to leak";
        CORE::exit(1);
    });
    ...
    my ($new_stage, $spawn_stage) = $self->spawn_stage_callback($spawn, $guard);
    ...
    $self->stage_exit($stage, $guard, $ok, $err);
}

sub stage_exit {
    my $self = shift;
    my ($stage, $guard, $ok, $err) = @_;
...
sub task_loop {
    ...
    while (1) {
        ...
        my $complete = { map { $_->run_id => 1 } $run->jobs->read };

        while (1) {
            return if $self->end_loop();

            my $task = $self->next($run, $stage) or last;
            next if $complete->{$task->{run_id}};
