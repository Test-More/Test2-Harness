package App::Yath2::Resource::SharedJobSlots::State;
use strict;
use warnings;
...
# Timeout a run's registration if it has not updated its 'seen' stamp in
# this many seconds. Matches old/: five minutes between heartbeats and
# the co-ordinator considers the runner gone.
sub TIMEOUT() { 300 }
...
sub _read_state {
    ...
    my $file = Test2::Harness2::Util::File::JSON->new(name => $self->{+STATE_FILE});
...
sub _allocate_slots {
    ...
    # deadlock with all runners holding an insufficient allocation.
    ...
    for my $try (0 .. 1) {
        ...
        # If we get here we have an allotment (not 0) but it does not meet the
...
sub _redistribute_fair {
    ...
    # Sort unmet runners by how much they already hold and by when they
    # were added; give the next slot to whoever holds the least so far.
    # O(n log n) per slot handed out; acceptable for typical runner
    # counts (handful per host) and matches old/'s behaviour.
    my @runners = values %{$state->{+RUNNERS}};
    while ($free > 0) {
        @runners = sort { $a->{allotment} <=> $b->{allotment} || $a->{added} <=> $b->{added} }
            grep { my $c = $self->_runner_calcs($_); $c->{wants} > $_->{allotment} } @runners;
...
App::Yath2::Resource::SharedJobSlots::State - shared state for job slots

=head1 DESCRIPTION

Owns the on-disk JSON state file that coordinates slot allocation
across multiple concurrent yath invocations on the same host. All
reads and writes are serialised by a sibling C<.LOCK> file held by
L<Fcntl/flock> while the caller is in a transaction.

The high-level API is C<allocate_slots>, C<assign_slots>,
C<release_slots>; each runs inside a C<rw> transaction that locks
the file, reloads state, runs the redistribution algorithm, writes
the result back, and releases the lock.

Two built-in redistribution algorithms are provided:
C<_redistribute_fair> (equal share with tie-break on age) and
C<_redistribute_first> (oldest registration wins). Custom algorithms
may be supplied as a fully-qualified C<Module::function> string in
the config file; see L<App::Yath2::Resource::SharedJobSlots> for the
contract.
