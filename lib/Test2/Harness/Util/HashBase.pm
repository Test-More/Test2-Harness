package Test2::Harness::Util::HashBase;
use strict;
use warnings;

our $VERSION = '1.000089';
...

{
    ...
    *Test2::Harness::Util::HashBase::CAN_CACHE = \%Object::HashBase::CAN_CACHE;
}
...
sub attr_list {
    ...
    my @list = grep { !$seen{$_}++ } map {
        ...
        if (0.004 > ($Test2::Harness::Util::HashBase::VERSION{$_} || 0)) {
            Carp::carp("$_ uses an inlined version of Test2::Harness::Util::HashBase too old to support attr_list()");
        }
        else {
            my $list = $Test2::Harness::Util::HashBase::ATTR_LIST{$_};
            @out = $list ? @$list : ()
        }
...
sub _new {
    ...
    $Test2::Harness::Util::HashBase::CAN_CACHE{$class} = $self->can('init')
        unless exists $Test2::Harness::Util::HashBase::CAN_CACHE{$class};

    $self->init if $Test2::Harness::Util::HashBase::CAN_CACHE{$class};
...
declare it before loading L<Test2::Harness::Util::HashBase>.
...
B<Note:> Test2::Harness::Util::HashBase checks for an init using C<< $class->can('init') >>
...
Test2::Harness::Util::HashBase provides a function for retrieving a list of attributes for an
Test2::Harness::Util::HashBase class.

=over 4

=item @list = Test2::Harness::Util::HashBase::attr_list($class)

=item @list = $class->Test2::Harness::Util::HashBase::attr_list()
