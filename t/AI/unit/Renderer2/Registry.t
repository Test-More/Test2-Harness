...
{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('terminal');
    is($class,  'App::Yath2::Renderer2::Terminal', 'terminal -> Terminal class');
    is($prefix, 'terminal',                        'terminal prefix');
}

{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('terminal-auto');
    is($class,  'App::Yath2::Renderer2::Terminal', 'terminal-auto -> Terminal class');
    is($prefix, 'terminal',                        'terminal-auto shares the terminal prefix');
}

{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('junit');
    is($class,  'App::Yath2::Renderer2::JUnit', 'junit -> JUnit class');
...
{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('+My::Cool::Renderer');
    is($class,  'My::Cool::Renderer', '+ prefix strips through');
    is($prefix, 'renderer',           'default prefix is lowercased tail');
}

{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('+My::Renderer::FooBar');
...
{
    my ($class, $prefix) = App::Yath2::Renderer2::Registry->resolve_name('some-other-thing');
    is($class,  'App::Yath2::Renderer2::SomeOtherThing', 'dashed -> CamelCase under Renderer2 namespace');
...
like(
    dies { App::Yath2::Renderer2::Registry->resolve_name(undef) },
    qr/name is required/,
    'undef name dies',
);
...
App::Yath2::Renderer2::Registry->_reset_prefix_table;

App::Yath2::Renderer2::Registry->assert_prefix('My::A', 'shared');
my $owners = App::Yath2::Renderer2::Registry->prefix_owners;
is($owners->{shared}, 'My::A', 'prefix owner recorded');

# Same class re-registering same prefix is fine.
ok(
    lives { App::Yath2::Renderer2::Registry->assert_prefix('My::A', 'shared') },
    'same class can re-assert same prefix',
);

# Different class trying to claim the same prefix is fatal at registration.
like(
    dies { App::Yath2::Renderer2::Registry->assert_prefix('My::B', 'shared') },
...
ok(
    lives { App::Yath2::Renderer2::Registry->assert_prefix('My::B', 'other') },
...
ok(
    lives { App::Yath2::Renderer2::Registry->include_all_renderer_options($opts) },
    'including all built-in renderers does not raise',
);

my $now = App::Yath2::Renderer2::Registry->prefix_owners;
is($now->{terminal}, 'App::Yath2::Renderer2::Terminal', 'terminal prefix registered');
is($now->{junit},    'App::Yath2::Renderer2::JUnit',    'junit prefix registered');
