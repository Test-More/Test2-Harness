...
# Inline resource that hosts one service_foo method. Records the
...
subtest 'consumes the IPC service + resource host roles' => sub {
    ...
    is($svc->_service_host_scope, 'run', 'scope is run');
    ok(ref($svc->_service_host_run), 'host run is the Run object');
};
