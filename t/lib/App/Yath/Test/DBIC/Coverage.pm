...
sub run_coverage_tests {
    ...
    # HARDCODED — fixture directory is the same for all backends.
    # The plan proposed deriving $dir from caller, but the new shim
    # filenames are `dbic-coverage-*.t` and the regex would yield
    # `t/integration/dbic-coverage`, which doesn't exist. The fixture
    # data all lives under `t/integration/coverage/`.
    my $dir = 't/integration/coverage';
