...
option_group {group => 'yathui', category => "YathUI Options"} => sub {
    option url => (
        type => 'Scalar',
        alt => ['uri'],
...
#    );
    ...
    option_post_process -1 => sub {
        ...
        my $has_finder = $options && $options->included->{'App::Yath::Options::Finder'};
        my $has_logger = $options && $options->included->{'App::Yath::Options::Logging'};
