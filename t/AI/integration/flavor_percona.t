...
BEGIN {
    ...
    my ($ok, $why) = DBIx::QuickDB::Driver::Percona->viable({bootstrap => 1, autostart => 1});
    skip_all "Percona not provisionable: $why" unless $ok;
}

my $h = Test2::Harness2->new(ephemeral => 'percona');
$h->initialize;

my $con = $h->connection;
ok($con, 'connected to ephemeral percona');

# project + run round-trip: exercises uuid, datetime, boolean paths.
my $proj = $con->handle('project')->insert({name => 'proj-percona'});
