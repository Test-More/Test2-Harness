package App::Yath::Options::Instance;
use strict;
use warnings;
...
sub process_option_post_actions {
    ...
    my $todo = delete $self->{+PENDING_POST} or return;

    for my $post_set (sort { $a->[0]->post_process_weight <=> $b->[0]->post_process_weight } reverse @$todo) {
        my ($opt, $post) = @$post_set;
        $post->(
            options  => $self,
            option   => $opt,
            args     => $self->{+ARGS},
...
sub _process_opts {
    ...
    for my $opt_set (@$list) {
        ...
        my $post = $opt->post_process // next;
        push @{$self->{+PENDING_POST} //= []} => [$opt, $post];
    }
}
...
sub _grab_opts {
    ...
    while (my $arg = shift @$args) {
        if ($ARG_ENDS{$arg}) {
            push @keep_args => $arg if $ARG_PASS{$arg};
            last;
        }
...
sub include_option {
    ...
    $self->_index_option($opt);
    $self->_list_option($opt);
