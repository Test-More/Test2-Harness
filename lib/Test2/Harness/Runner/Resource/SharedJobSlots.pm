...
Config file: $config_file
    ...
    $self->{+STATE} = Test2::Harness::Runner::Resource::SharedJobSlots::State->new(
        ...
        max_slots         => $host_conf->{max_slots}         // die("'max_slots' not set in '$config_file' for host '$host'.\n"),
        max_slots_per_job => $host_conf->{max_slots_per_job} // die("'max_slots_per_job' not set in '$config_file' for host '$host'.\n"),
        max_slots_per_run => $host_conf->{max_slots_per_run} // die("'max_slots_per_run' not set in '$config_file' for host '$host'.\n"),
        request_sort      => $sort,
    );
...
sub available {
    ...
    my $concurrency = min(grep { $_ } ($task->{slots_per_job} // 1), @{$self->{+CONFIG}}{qw/max_slots_per_run max_slots_per_job/});
    $concurrency ||= 1;
