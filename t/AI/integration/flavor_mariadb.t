...
BEGIN {
    ...
    eval { require DBD::MariaDB; 1 } or eval { require DBD::mysql; 1 }
        or skip_all "neither DBD::MariaDB nor DBD::mysql available";
    require DBIx::QuickDB::Driver::MariaDB;
    # viable() takes a single hashref; the 2-arg form crashes the driver.
    my ($ok, $why) = DBIx::QuickDB::Driver::MariaDB->viable({bootstrap => 1, autostart => 1});
    skip_all "MariaDB not provisionable: $why" unless $ok;
}

my $h = Test2::Harness2->new(ephemeral => 'mariadb');
$h->initialize;

my $con = $h->connection;
ok($con, 'connected to ephemeral mariadb');

# project + run round-trip: exercises uuid, datetime, boolean paths.
my $proj = $con->handle('project')->insert({name => 'proj-mariadb'});
