...
# Pure-logic test of PreloadRouter::resolve_for_job. We build a tiny
# harness stub (bless { resources => [...] }, 'Test2::Harness2') and
# stub Run / Job / TestFile shims so we never spin up a real harness.
# The resolver only reads from $self->{resources} +
# $job->test_file->preload_preferences + $run->run_id.
...
sub make_harness {
    ...
    # The resolver lives on PreloadRouter; bolt a stub router onto the
    # harness so $h->preload_router->resolve_for_job(...) reaches it.
    # The router only reads the harness's RESOURCES slot via its
    # harness backref, so a stub blessed into the right class with a
    # weakened harness ref does not need full ctor wiring.
    my $router = bless {
        harness                => $h,
...
subtest '<no> returns no_preload' => sub {
    my $h = make_harness();
    my @r = $h->preload_router->resolve_for_job(make_run(), make_job('<no>'));
...
subtest 'empty list defaults to <default> -> no_preload when no default exists' => sub {
    my $h = make_harness();
    my @r = $h->preload_router->resolve_for_job(make_run(), make_job('<default>'));
...
subtest 'name match: per-run takes precedence over global' => sub {
    ...
    my @r = $h->preload_router->resolve_for_job(make_run('R1'), make_job('mypl'));
...
subtest 'name match: global usable' => sub {
    ...
    my @r  = $h->preload_router->resolve_for_job(make_run(), make_job('foo'));
...
subtest 'permanent_broken skips to next entry' => sub {
    ...
    my @r = $h->preload_router->resolve_for_job(make_run(), make_job('a', 'b'));
...
subtest 'all candidates permanent_broken returns broken + first name' => sub {
    ...
    my @r = $h->preload_router->resolve_for_job(make_run(), make_job('a'));
...
subtest 'unknown name is broken + first name' => sub {
    my $r1 = make_preload(name => 'foo', scope => 'global', usable => 1);
    my $h  = make_harness($r1);
    my @r  = $h->preload_router->resolve_for_job(make_run(), make_job('bar'));
...
subtest 'unknown name followed by <no> falls through to no_preload' => sub {
    my $r1 = make_preload(name => 'foo', scope => 'global', usable => 1);
    my $h  = make_harness($r1);
    my @r  = $h->preload_router->resolve_for_job(make_run(), make_job('bar', '<no>'));
...
subtest 'permanent-broken followed by <no> falls through to no_preload' => sub {
    ...
    my $h  = make_harness($r1);
    my @r  = $h->preload_router->resolve_for_job(make_run(), make_job('foo', '<no>'));
...
subtest '<default>: per-run takes precedence over global' => sub {
    ...
    my @r = $h->preload_router->resolve_for_job(make_run('R1'), make_job('<default>'));
