package App::Yath::Resource::SharedJobSlots::Config;
use strict;
use warnings;
...
sub algorithm {
    ...
    if ($algorithm =~ m/^(.*)::([^:]+)$/) {
        ...
        require App::Yath::Resource::SharedJobSlots::State;
        ...
        die "'$short' is not a valid algorithm (in file '$self->{+CONFIG_FILE}' under host '$self->{+HOST}' key 'algorithm'). Must be 'fair', 'first', or a Fully::Qualified::Module::function_name."
            unless App::Yath::Resource::SharedJobSlots::State->can($algorithm);
    }
...
App::Yath::Resource::SharedJobSlots::Config - Config for shared job slots
