...
sub import {
    ...
    $exports{watch} = sub {
        ...
        if (my $stage = $Test2::Harness2::PreloadService::Stage::ACTIVE) {
            return $stage->watch(@_);
        }
...
sub build_stage {
    ...
    my $stage = Test2::Harness2::Preload::Stage->new(%params);
...
one is running, so application code can add watches dynamically. Reloader
support arrives in Stage 9; until then the fallback path is inert.
